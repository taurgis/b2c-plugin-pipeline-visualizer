...
function parsePipeline(xml, sourceName = "pipeline.xml") {
    ...
    const nodes = [];
    const edges = [];
    const segmentFirstNodeMap = new Map();
    const deferredEdges = [];
    let branchOrdinal = 0;
    ...
    function parseBranchWithEntry(branchEl, branchPath, parentLoopNodeId) {
        const entryIds = [];
        let segmentIndex = 0;
    ...
    function parseSegment(segmentEl, branchPath, segmentIndex, parentLoopNodeId) {
        let lastNodeId;
        let firstNodeId;
        let pendingLabel;
        let pendingSourceConnector;
        let pendingTargetConnector;
        let pendingDisplay;
    ...
    function parseNode(nodeEl, branchPath, segmentIndex, nodeIndex) {
        ...
        for (const nestedBranch of getElementChildren(nodeEl, "branch")) {
            const connectorLabel = nestedBranch.getAttribute("source-connector") ||
            ...
            const branchResult = parseBranchWithEntry(nestedBranch, nestedPath, isLoopNode ? id : undefined);
    ...
    function processDeferredEdges() {
        for (const deferred of deferredEdges) {
            const targetNodeId = resolveTargetPath(deferred.targetPath, deferred.fromBranchPath, deferred.fromSegmentIndex);
            if (targetNodeId) {
                edges.push({
    ...
    function resolveTargetPath(targetPath, currentBranchPath, currentSegmentIndex) {
...
function getElementChildren(parent, tagFilter) {
    const elements = [];
...
function findFirstElement(parent, predicate) {
    for (let i = 0; i < parent.childNodes.length; i += 1) {
        const node = parent.childNodes.item(i);
        if (node && node.nodeType === 1) {
            const el = node;
...
function readFirstChildText(parent, tagName) {
...
function collectConfigProperties(el) {
    const properties = [];
...
function collectKeyBindings(el) {
    const bindings = [];
    for (let i = 0; i < el.childNodes.length; i += 1) {
        const node = el.childNodes.item(i);
        if (node && node.nodeType === 1) {
...
function deriveTransitionLabel(el) {
    return (el.getAttribute("source-connector") ||
        el.getAttribute("target-connector") ||
        el.getAttribute("condition-key") ||
        el.getAttribute("name") ||
        undefined);
}
function parseTransitionDisplay(transitionEl) {
...
function getSourceLocation(el) {
    ...
    const columnNumber = el.columnNumber;
